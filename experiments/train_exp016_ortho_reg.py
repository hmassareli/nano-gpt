...
"""
EXP_TITLE = "EXP-016: Baseline + Soft Ortho Regularization on LM Head"
...
class GPT(nn.Module):
    ...
    def forward_head(self, x, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            # EXP-016: Soft orthogonality regularization on LM head
            # Encourages W^T W ≈ I_D so gradient projection preserves rank & norm
            W = self.lm_head.weight  # (V, D)
            WtW = W.float().T @ W.float()  # (D, D)
            I = torch.eye(WtW.shape[0], device=WtW.device)
            ortho_loss = ((WtW - I) ** 2).mean()
...
# EXP-016 specific
ORTHO_LAMBDA = 0.01      # weight for soft orthogonality regularization on LM head
