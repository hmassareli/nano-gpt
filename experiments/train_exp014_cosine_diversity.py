...
"""
EXP_TITLE = "EXP-014: 4-Head + Cosine Diversity (4x D->128->GELU->V)"
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        # EXP-014: Sum logits from all output subheads + cosine diversity penalty
        softcap = 15
        logits = sum(head(x) for head in self.output_heads)
        logits = logits.float()
        ...
        if targets is not None:
            ...
            # Cosine diversity penalty: penalize pairwise similarity of proj weights
            heads = self.output_heads
            n_h = len(heads)
            cos_penalty = 0.0
            for i in range(n_h):
                for j in range(i + 1, n_h):
                    wi = heads[i].proj.weight.flatten()
                    wj = heads[j].proj.weight.flatten()
                    cos_penalty = cos_penalty + F.cosine_similarity(wi.unsqueeze(0), wj.unsqueeze(0)).squeeze()
            cos_penalty = cos_penalty / max(n_h * (n_h - 1) / 2, 1)
            loss = loss + DIVERSITY_LAMBDA * cos_penalty
            return loss
        return logits
...
DIVERSITY_LAMBDA = 0.1   # weight for cosine diversity penalty
