...
while True:
    ...
    if TWO_STAGE_HEAD_UPDATE:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with autocast_ctx:
                h = hiddens.detach()  # (B, T, D) — detach backbone to avoid double backward
                h_norm = F.normalize(h.view(-1, h.size(-1)), dim=-1)  # (B*T, D)
                sim = h_norm @ h_norm.T  # (B*T, B*T)
                targets_flat = y.view(-1)  # (B*T,)
                # Mask: same target token = positive pair
                mask = (targets_flat.unsqueeze(0) == targets_flat.unsqueeze(1)).float()
