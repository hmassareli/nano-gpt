...
"""
EXP_TITLE = "EXP-024: Temperature Schedule (high->low)"
...
class GPT(nn.Module):
    ...
    def forward_head(self, x, targets=None, reduction='mean', temperature=1.0):
        softcap = 13
        logits = self.lm_head(x)
        logits = logits.float()
        logits = softcap * torch.tanh(logits / softcap)
        # EXP-024: Apply temperature scaling
    ...
    def forward(self, idx, targets=None, reduction='mean', temperature=1.0):
        x = self.forward_backbone(idx)
        return self.forward_head(x, targets, reduction, temperature)
...
# EXP-024 specific
TEMP_START = 3.0         # initial temperature (high = spread gradient)
TEMP_END = 1.0           # final temperature (normal)
TEMP_WARMDOWN = 0.5      # fraction of training to reach final temp
...
while True:
    ...
    # EXP-024: Temperature schedule
    if progress < TEMP_WARMDOWN:
        temp = TEMP_START + (TEMP_END - TEMP_START) * (progress / TEMP_WARMDOWN)
    else:
        temp = TEMP_END

    for micro_step in range(grad_accum_steps):
        with autocast_ctx:
            loss = model(x, y, temperature=temp)
