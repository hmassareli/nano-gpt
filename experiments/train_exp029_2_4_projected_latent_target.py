...
"""
EXP_TITLE = "EXP-029.2.4: Next-Token Projected Latent Target"
...
PROJECTED_TARGET_DIM = 256  # Auxiliary target lives in a smaller, head-adjacent subspace
...
class TargetProjector(nn.Module):
    """Map final hidden states into a smaller auxiliary prediction space."""
...
class GPT(nn.Module):
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, return_components=False):
        ...
        if targets is not None:
            ...
            # EXP-029.2.4: predict a smaller projected target rather than the raw final hidden state
            if intermediate is not None and intermediate.size(1) > 1:
                predicted_next = self.latent_predictor(intermediate[:, :-1].float())
                target_next = self.target_projector(x[:, 1:].float().detach())
