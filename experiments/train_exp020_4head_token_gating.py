...
"""
EXP_TITLE = "EXP-020: 4-Head No GELU + Token Gating + Load Balance"
...
# EXP-020: Linear output subhead kept intentionally simple.
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.head_gate = nn.Linear(config.n_embd, config.num_output_heads, bias=False)
    ...
    def num_scaling_params(self):
        ...
        # EXP-020: count output heads plus gating network.
        lm_head = sum(p.numel() for p in self.output_heads.parameters()) + sum(p.numel() for p in self.head_gate.parameters())
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        # EXP-020: all output head and gate params get unembedding LR
        lm_head_params = list(self.output_heads.parameters()) + list(self.head_gate.parameters())
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        # EXP-020: Token-wise gating keeps heads distinct even without GELU.
        softcap = 15
        raw_head_logits = [head(x).float() for head in self.output_heads]
        gate_logits = self.head_gate(x).float()
        gate_probs = F.softmax(gate_logits, dim=-1)
        logits = 0.0
        for i, head_logits_i in enumerate(raw_head_logits):
            logits = logits + gate_probs[:, :, i:i+1] * head_logits_i
        ...
        if targets is not None:
            ...
            aux_loss = 0.0
            for head_logits_i in raw_head_logits:
                aux_logits = softcap * torch.tanh(head_logits_i / softcap)
            ...
            aux_loss = aux_loss / max(len(raw_head_logits), 1)
            avg_gate = gate_probs.mean(dim=(0, 1))
            uniform = torch.full_like(avg_gate, 1.0 / avg_gate.numel())
            load_balance_loss = (avg_gate - uniform).pow(2).mean()
            loss = loss + AUX_HEAD_LAMBDA * aux_loss + LOAD_BALANCE_LAMBDA * load_balance_loss
            return loss
        return logits
...
AUX_HEAD_LAMBDA = 0.2    # keeps every head individually useful
LOAD_BALANCE_LAMBDA = 0.1 # prevents the gate from collapsing to one head
