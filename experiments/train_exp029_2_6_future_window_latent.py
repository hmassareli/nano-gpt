...
"""
EXP_TITLE = "EXP-029.2.6: Future-Window Latent Target"
...
FUTURE_WINDOW = 4  # Predict a short-horizon summary rather than a single next hidden state
...
class GPT(nn.Module):
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, return_components=False):
        ...
        if targets is not None:
            ...
            # EXP-029.2.6: predict a short-horizon future summary instead of a single future hidden state
            if intermediate is not None and intermediate.size(1) > FUTURE_WINDOW:
                ...
                target_next = torch.stack(target_terms, dim=0).mean(dim=0)
