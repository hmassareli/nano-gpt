...
"""
EXP_TITLE = "EXP-002: Spectral Init (SVD of wte)"
...
# EXP-002: Spectral Head Init — initialize lm_head from SVD of wte
with torch.no_grad():
    wte_w = model.transformer.wte.weight.float()  # (V, D)
    U, S, Vh = torch.linalg.svd(wte_w, full_matrices=False)  # U(V,D), S(D), Vh(D,D)
    # Pseudo-inverse scaled to match original head norm
    S_inv = 1.0 / S.clamp(min=1e-6)
    head_init = (Vh.T * S_inv[None, :]) @ U.T  # (D, V)
    # Scale to match the original init norm
    original_norm = model.lm_head.weight.data.norm()
    head_init = head_init.T * (original_norm / head_init.norm())  # (V, D)
    model.lm_head.weight.data.copy_(head_init.to(model.lm_head.weight.dtype))
    print(f"EXP-002: Spectral head init applied (SVD of wte, shape {wte_w.shape})")
