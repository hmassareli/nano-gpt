...
"""
EXP_TITLE = "EXP-026.4: Token-Patch Codec Target (Warmdown)"
...
# ---------------------------------------------------------------------------
# EXP-026.4 hyperparameters
# ---------------------------------------------------------------------------
SOURCE_LAYER = 6  # Predict from h_6
LATENT_LOSS_LAMBDA = 0.25  # Match the best fixed auxiliary regime before warmdown
PREDICTOR_HIDDEN_DIM = 512  # Small MLP predictor
CODEC_HIDDEN_DIM = 512  # Keep the codec modest so it does not become a second backbone
CODEC_LATENT_DIM = 256  # Narrow token-patch latent target
PATCH_SIZE = 4  # Future token chunk size, mirroring the CALM-style patch idea
CODEC_PRETRAIN_END = 0.15  # First train the token-patch codec, then freeze it
LATENT_WARMDOWN_START = 0.25  # Start decaying latent pressure after the early gain phase
...
def latent_lambda_for_progress(progress):
    if progress < CODEC_PRETRAIN_END:
        return 0.0
...
class LatentPredictor(nn.Module):
    """Small MLP to predict a compact future latent target from the source hidden state."""
...
class TokenPatchCodec(nn.Module):
    """Encode a future patch of tokens into a compact latent and reconstruct the patch."""
    def __init__(self, vocab_size, n_embd, hidden_dim, latent_dim, patch_size):
        super().__init__()
        self.patch_size = patch_size
        self.token_embed = nn.Embedding(vocab_size, n_embd)
        self.encoder_fc1 = nn.Linear(patch_size * n_embd, hidden_dim, bias=False)
        self.encoder_fc2 = nn.Linear(hidden_dim, latent_dim, bias=False)
        self.decoder_fc1 = nn.Linear(latent_dim, hidden_dim, bias=False)
        self.decoder_fc2 = nn.Linear(hidden_dim, patch_size * n_embd, bias=False)

    def encode(self, token_patches):
        patch_embeds = self.token_embed(token_patches)
        patch_embeds = patch_embeds.reshape(*token_patches.shape[:-1], -1)
        patch_hidden = self.encoder_fc1(patch_embeds)
        patch_hidden = F.gelu(patch_hidden)
        latent = self.encoder_fc2(patch_hidden)
        return latent

    def decode(self, latent):
        patch_hidden = self.decoder_fc1(latent)
        patch_hidden = F.gelu(patch_hidden)
        decoded = self.decoder_fc2(patch_hidden)
        decoded = decoded.view(*latent.shape[:-1], self.patch_size, self.token_embed.embedding_dim)
        return decoded

    def reconstruction_loss(self, token_patches):
        latent = self.encode(token_patches)
        decoded = self.decode(latent)
        logits = F.linear(decoded.float(), self.token_embed.weight.float())
        loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), token_patches.reshape(-1), reduction='mean')
        return latent, loss
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # EXP-026.4: Predict a frozen token-patch latent target from an intermediate hidden state
        self.latent_predictor = LatentPredictor(config.n_embd, PREDICTOR_HIDDEN_DIM, CODEC_LATENT_DIM)
        self.token_patch_codec = TokenPatchCodec(config.vocab_size, config.n_embd, CODEC_HIDDEN_DIM, CODEC_LATENT_DIM, PATCH_SIZE)
        self.codec_trainable = True
    ...
    def init_weights(self):
        ...
        # EXP-026.4: Init latent modules
        torch.nn.init.uniform_(self.latent_predictor.fc1.weight, -s, s)
        torch.nn.init.zeros_(self.latent_predictor.fc2.weight)
        torch.nn.init.normal_(self.token_patch_codec.token_embed.weight, mean=0.0, std=1.0)
        torch.nn.init.uniform_(self.token_patch_codec.encoder_fc1.weight, -s, s)
        torch.nn.init.uniform_(self.token_patch_codec.encoder_fc2.weight, -s, s)
        torch.nn.init.uniform_(self.token_patch_codec.decoder_fc1.weight, -s, s)
        torch.nn.init.zeros_(self.token_patch_codec.decoder_fc2.weight)
    ...
    def set_codec_trainable(self, trainable):
        self.codec_trainable = trainable
        for param in self.token_patch_codec.parameters():
            param.requires_grad = trainable

    def _future_token_patches(self, targets):
        if targets is None or targets.size(1) < PATCH_SIZE:
            return None, None
        token_patches = targets.unfold(dimension=1, size=PATCH_SIZE, step=1)
        valid_mask = token_patches.ne(-1).all(dim=-1)
        return token_patches, valid_mask
    ...
    def num_scaling_params(self):
        ...
        token_patch_codec = sum(p.numel() for p in self.token_patch_codec.parameters())
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel()
        total = wte + value_embeds + lm_head + transformer_matrices + latent_predictor + token_patch_codec + scalars
        return {
            'wte': wte, 'value_embeds': value_embeds, 'lm_head': lm_head,
            'transformer_matrices': transformer_matrices, 'latent_predictor': latent_predictor,
            'token_patch_codec': token_patch_codec,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        codec_params = list(self.token_patch_codec.parameters())
    ...
    def forward_backbone(self, idx, return_intermediate=False):
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            # EXP-029.2: Save intermediate hidden state from source layer
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, return_components=False, latent_lambda=None):
        ...
        if targets is not None:
            if latent_lambda is None:
                latent_lambda = 0.0 if not self.training else LATENT_LOSS_LAMBDA
            ...
            token_patches, valid_mask = self._future_token_patches(targets)
            if intermediate is not None and token_patches is not None:
                valid_len = token_patches.size(1)
                predicted_latent = self.latent_predictor(intermediate[:, :valid_len].float())
                if valid_mask is not None and valid_mask.any():
                    flat_patches = token_patches[valid_mask]
                    flat_predicted = predicted_latent[valid_mask]
                    if self.codec_trainable and self.training:
                        _, codec_loss = self.token_patch_codec.reconstruction_loss(flat_patches)
                    if latent_lambda > 0.0:
                        with torch.no_grad():
                            target_latent = self.token_patch_codec.encode(flat_patches).float()
                        latent_loss = F.mse_loss(
                            F.normalize(flat_predicted, dim=-1),
                            F.normalize(target_latent, dim=-1),
                            reduction='mean'
                        )
            total_loss = ce_loss + codec_loss + latent_lambda * latent_loss
            if return_components:
                return (
                    total_loss,
                    ce_loss.detach().float(),
                    latent_loss.detach().float(),
                    codec_loss.detach().float(),
                    torch.tensor(latent_lambda, device=logits.device, dtype=torch.float32),
                )
            return total_loss
        return logits
...
while True:
    ...
    codec_trainable = progress < CODEC_PRETRAIN_END
    ...
    for micro_step in range(GRAD_ACCUM_STEPS):
        with autocast_ctx:
            ...
            train_loss, ce_loss, latent_loss, codec_loss, latent_lambda_t = model(x, y, reduction='sum', return_components=True, latent_lambda=latent_lambda)
    ...
    codec_phase = "codec" if codec_trainable else "frozen"
    print(f"step {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | ce: {debiased_smooth_ce:.6f} | latent: {debiased_smooth_latent:.6f} | codec: {debiased_smooth_codec:.6f} | latent_lambda: {debiased_smooth_latent_lambda:.4f} | codec_phase: {codec_phase} | lrm: {lrm:.2f} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.1f}% | epoch: {epoch} | remaining: {remaining_str}    ", flush=True)
...
if not SKIP_EVAL:
    model.eval()
    model.set_codec_trainable(False)
