...
"""
EXP_TITLE = "EXP-029.3.1: EMA Projected Future-Window Latent Target"
...
# ---------------------------------------------------------------------------
# EXP-029.3.1 hyperparameters
# ---------------------------------------------------------------------------
SOURCE_LAYER = 6  # Predict from h_6
LATENT_LOSS_LAMBDA = 0.25  # Match the strongest 029.2 fixed auxiliary regime
PREDICTOR_HIDDEN_DIM = 512  # Small MLP predictor
PROJECTED_TARGET_DIM = 256  # Narrow target space to keep the bypass head-adjacent
FUTURE_WINDOW = 4  # Predict a short-horizon future summary instead of one hidden state
EMA_MOMENTUM = 0.996  # EMA momentum for teacher (higher = slower update)
LATENT_WARMDOWN_START = 0.2  # Start decaying the auxiliary signal after the early help phase
LATENT_WARMDOWN_END = 0.9    # Remove most latent pressure by the late CE-only regime
...
class TargetProjector(nn.Module):
    """Map teacher future summaries into a smaller auxiliary prediction space."""
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # EXP-029.3.1: EMA-stabilized projected future-window predictor.
        self.latent_predictor = LatentPredictor(config.n_embd, PREDICTOR_HIDDEN_DIM, PROJECTED_TARGET_DIM)
    ...
    def init_weights(self):
        ...
        # EXP-029.3.1: Init latent modules.
    ...
    def forward_backbone(self, idx, return_intermediate=False):
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            # EXP-029.3: Save intermediate hidden state from source layer
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, teacher_target=None, return_components=False, latent_lambda=None):
        softcap = 13
        ...
        if targets is not None:
            ...
            # EXP-029.3.1: EMA teacher supplies a projected short-horizon future summary.
            if intermediate is not None and teacher_target is not None and teacher_target.size(1) > FUTURE_WINDOW:
                valid_len = teacher_target.size(1) - FUTURE_WINDOW
                predicted_final = self.latent_predictor(intermediate[:, :valid_len].float())
                target_terms = [
                    teacher_target[:, offset:offset + valid_len].float().detach()
                    for offset in range(1, FUTURE_WINDOW + 1)
                ]
                future_summary = torch.stack(target_terms, dim=0).mean(dim=0)
                target_projected = self.target_projector(future_summary)
                pred_norm = F.normalize(predicted_final, dim=-1)
                target_norm = F.normalize(target_projected, dim=-1)
                latent_loss = (1.0 - (pred_norm * target_norm).sum(dim=-1)).mean()
    ...
    def forward(self, idx, targets=None, reduction='mean', teacher_target=None, return_components=False, latent_lambda=None):
        if targets is not None:
            x, intermediate = self.forward_backbone(idx, return_intermediate=True)
            return self.forward_head(x, targets, reduction, intermediate, teacher_target, return_components, latent_lambda)
        else:
            x = self.forward_backbone(idx, return_intermediate=False)
            return self.forward_head(x, targets, reduction, None, None, False)
...
class EMATeacher:
    ...
    def get_target(self, idx):
        """Get teacher's final hidden state (stop-grad by default)."""
        return self.teacher.forward_backbone(idx, return_intermediate=False)
...
# EXP-029.3: Setup EMA teacher
teacher = EMATeacher(model, momentum=EMA_MOMENTUM)
print(f"EMA teacher initialized with momentum={EMA_MOMENTUM}")
...
while True:
    ...
    for micro_step in range(GRAD_ACCUM_STEPS):
        with autocast_ctx:
            # EXP-029.3: Get teacher target (stop-grad by default)
            with torch.no_grad():
                teacher_target = teacher.get_target(x).float()

            latent_lambda = latent_lambda_for_progress(progress)
            train_loss, ce_loss, latent_loss, latent_lambda_t = model(
                x,
                y,
                reduction='sum',
    ...
    # EXP-029.3: Update teacher with EMA
