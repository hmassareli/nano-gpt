...
"""
EXP_TITLE = "EXP-022.3: Learned-Projection Embedding Bypass"
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.target_projection = nn.Linear(config.n_embd, config.n_embd, bias=False)
        # Value embeddings
    ...
    def init_weights(self):
        ...
        torch.nn.init.eye_(self.target_projection.weight)
        # Value embeddings
    ...
    def num_scaling_params(self):
        ...
        target_projection = sum(p.numel() for p in self.target_projection.parameters())
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel()
        total = wte + value_embeds + lm_head + transformer_matrices + target_projection + scalars
        return {
            'wte': wte, 'value_embeds': value_embeds, 'lm_head': lm_head,
            'transformer_matrices': transformer_matrices, 'target_projection': target_projection,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        target_projection_params = list(self.target_projection.parameters())
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(lm_head_params) + len(value_embeds_params) + len(resid_params) +
            len(x0_params) + len(target_projection_params))
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=target_projection_params, lr=0.02 * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward_head(self, x, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            # EXP-022.3: learned target projection bridges embedding geometry mismatch.
            ...
            if valid_mask.any():
                ...
                projected_targets = self.target_projection(target_embs[valid_mask])
                emb_sq_error = (flat_hidden[valid_mask] - projected_targets).pow(2).mean(dim=-1)
                if reduction == 'sum':
...
# EXP-022 specific
EMB_LOSS_LAMBDA = 0.1    # weight for learned-projection embedding bypass
