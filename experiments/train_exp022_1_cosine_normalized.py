...
"""
EXP_TITLE = "EXP-022.1: Cosine-Normalized Embedding Loss"
...
class GPT(nn.Module):
    ...
    def forward_head(self, x, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            # EXP-022.1: cosine-normalized supervision in embedding space.
            ...
            if valid_mask.any():
                ...
                hidden_norm = F.normalize(flat_hidden[valid_mask], dim=-1)
                target_norm = F.normalize(target_embs[valid_mask], dim=-1)
                emb_sq_error = 1 - (hidden_norm * target_norm).sum(dim=-1)
                if reduction == 'sum':
...
# EXP-022 specific
EMB_LOSS_LAMBDA = 0.1    # weight for cosine-normalized embedding loss
