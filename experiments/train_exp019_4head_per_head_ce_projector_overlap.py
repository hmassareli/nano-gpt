...
"""
EXP_TITLE = "EXP-019: 4-Head No GELU + Per-Head CE + Projector Overlap"
...
# EXP-019: Linear output subhead kept intentionally simple.
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        # EXP-019: Sum logits, keep each head predictive, and discourage subspace overlap.
        softcap = 15
        head_logits = [head(x).float() for head in self.output_heads]
        ...
        if targets is not None:
            ...
            # Penalize overlap between the row-spaces of projection matrices.
            heads = self.output_heads
            n_h = len(heads)
            overlap_penalty = 0.0
            head_bases = []
            for head in heads:
                basis, _ = torch.linalg.qr(head.proj.weight.float().T, mode='reduced')
                head_bases.append(basis)
            for i in range(n_h):
                for j in range(i + 1, n_h):
                    overlap = head_bases[i].T @ head_bases[j]
                    overlap_penalty = overlap_penalty + overlap.pow(2).mean()
            overlap_penalty = overlap_penalty / max(n_h * (n_h - 1) / 2, 1)
            loss = loss + DIVERSITY_LAMBDA * overlap_penalty
            return loss
        return logits
...
AUX_HEAD_LAMBDA = 0.25   # keeps every head individually useful
DIVERSITY_LAMBDA = 0.1   # weight for projector-overlap penalty
