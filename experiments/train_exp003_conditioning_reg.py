...
"""
EXP_TITLE = "EXP-003: Head Conditioning Regularization"
...
while True:
    ...
    if TWO_STAGE_HEAD_UPDATE:
        ...
        # Penalize deviation from orthogonality to keep effective dimension high
        HEAD_COND_LAMBDA = 0.01
        W = model.lm_head.weight.float()  # (V, D)
        WtW = W.T @ W  # (D, D)
        eye = torch.eye(WtW.shape[0], device=WtW.device)
        target_scale = WtW.diagonal().mean()
        cond_loss = HEAD_COND_LAMBDA * ((WtW / target_scale - eye) ** 2).mean()
        cond_loss.backward()
