...
"""
EXP_TITLE = "EXP-025: 4-Head No GELU + Per-Head CE + Hidden-Grad Diversity"
...
# EXP-025: Linear output subhead kept intentionally simple.
...
class GPT(nn.Module):
    ...
    def compute_output_logits(self, x):
        softcap = 15
        head_logits = [softcap * torch.tanh(head(x).float() / softcap) for head in self.output_heads]
        logits = sum(head_logits)
        return head_logits, logits
    ...
    def forward(self, idx, targets=None, reduction='mean', return_hidden=False):
        ...
        # EXP-025: Sum logits from all output subheads and supervise each head individually.
        head_logits, logits = self.compute_output_logits(x)

        if targets is not None:
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1),
                                   ignore_index=-1, reduction=reduction)
            aux_loss = 0.0
            for head_logits_i in head_logits:
            ...
            aux_loss = aux_loss / max(len(head_logits), 1)
            loss = loss + AUX_HEAD_LAMBDA * aux_loss
            if return_hidden:
                return loss, x
            return loss
        return logits
...
AUX_HEAD_LAMBDA = 0.25   # forces each head to remain individually predictive
GRAD_DIVERSITY_LAMBDA = 0.05  # penalty on pairwise similarity of dL_head/dh
GRAD_DIVERSITY_SUBSAMPLE = 64 # token positions sampled per micro-step for second-order penalty
...
print(f"Grad diversity: lambda={GRAD_DIVERSITY_LAMBDA} | subsample={GRAD_DIVERSITY_SUBSAMPLE}")
...
while True:
    ...
    ve_warmup_frac = min(progress / 0.07, 1.0) if progress < 0.07 else 1.0  # 7% VE warmup
    ...
    for micro_step in range(grad_accum_steps):
        ...
        valid_idx = (y.view(-1) != -1).nonzero(as_tuple=False).squeeze(-1)
        grad_div_loss = loss.new_zeros(())
        if valid_idx.numel() > 1:
            if valid_idx.numel() > GRAD_DIVERSITY_SUBSAMPLE:
                perm = torch.randperm(valid_idx.numel(), device=valid_idx.device)[:GRAD_DIVERSITY_SUBSAMPLE]
                sample_idx = valid_idx[perm]
            else:
                sample_idx = valid_idx
            sampled_hidden = hidden.view(-1, hidden.size(-1))[sample_idx].float()
            sampled_targets = y.view(-1)[sample_idx]
            sampled_head_logits, _ = model.compute_output_logits(sampled_hidden)
            head_grads = []
            for head_logits_i in sampled_head_logits:
                head_loss = F.cross_entropy(head_logits_i, sampled_targets, reduction='mean')
            ...
            if len(head_grads) >= 2:
                pair_count = 0
                grad_div_loss = loss.new_zeros(())
                for i in range(len(head_grads)):
                    gi = head_grads[i].reshape(-1)
                    for j in range(i + 1, len(head_grads)):
                        gj = head_grads[j].reshape(-1)
                        cos = F.cosine_similarity(gi.unsqueeze(0), gj.unsqueeze(0)).squeeze()
                        grad_div_loss = grad_div_loss + cos.square()
                        pair_count += 1
                grad_div_loss = grad_div_loss / max(pair_count, 1)

        total_loss = loss + GRAD_DIVERSITY_LAMBDA * grad_div_loss
