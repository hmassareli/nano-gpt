...
"""
EXP_TITLE = "EXP-005.1: Contrastive Loss With Projection Head"
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.contrastive_projection = ContrastiveProjectionHead(config.n_embd, CONTRASTIVE_PROJ_HIDDEN_DIM)
        # Value embeddings
    ...
    def init_weights(self):
        ...
        torch.nn.init.uniform_(self.contrastive_projection.fc1.weight, -s, s)
        torch.nn.init.zeros_(self.contrastive_projection.fc2.weight)
        # Value embeddings
    ...
    def num_scaling_params(self):
        ...
        contrastive_projection = sum(p.numel() for p in self.contrastive_projection.parameters())
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel()
        total = wte + value_embeds + lm_head + transformer_matrices + contrastive_projection + scalars
        return {
            'wte': wte, 'value_embeds': value_embeds, 'lm_head': lm_head,
            'transformer_matrices': transformer_matrices, 'contrastive_projection': contrastive_projection,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        projection_params = list(self.contrastive_projection.parameters())
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(lm_head_params) + len(value_embeds_params) + len(resid_params) +
            len(x0_params) + len(projection_params))
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=projection_params, lr=0.02 * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    if TWO_STAGE_HEAD_UPDATE:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with autocast_ctx:
                ...
                h_sub = model.contrastive_projection(h_sub.float())
                h_sub = h_sub.to(dtype=hiddens.dtype)
