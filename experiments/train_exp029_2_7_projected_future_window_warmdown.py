...
"""
EXP_TITLE = "EXP-029.2.7: Projected Future-Window Latent Prediction (Warmdown)"
...
# ---------------------------------------------------------------------------
# EXP-029.2.7 hyperparameters
# ---------------------------------------------------------------------------
SOURCE_LAYER = 6  # Predict from h_6
LATENT_LOSS_LAMBDA = 0.25  # Best fixed regime so far for the 029.2 family
...
FUTURE_WINDOW = 4  # Predict a short-horizon summary instead of a single next hidden state
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # EXP-029.2.7: Projected future-window predictor
    ...
    def init_weights(self):
        ...
        # EXP-029.2.7: Init latent modules
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, return_components=False, latent_lambda=None):
        ...
        if targets is not None:
            ...
            # EXP-029.2.7: projected short-horizon future summary with a late-stage warmdown
            if intermediate is not None and intermediate.size(1) > FUTURE_WINDOW:
                ...
                target_next = self.target_projector(future_summary)
