...
"""
EXP_TITLE = "EXP-029.2.5: Next-Token Logit Distillation"
...
# ---------------------------------------------------------------------------
# EXP-029.2 hyperparameters
# ---------------------------------------------------------------------------
SOURCE_LAYER = 6  # Predict from h_6
LATENT_LOSS_LAMBDA = 0.25  # Start from the most promising fixed-weight EXP-029.2.x regime
PREDICTOR_HIDDEN_DIM = 512  # Small MLP predictor
DISTILL_TEMPERATURE = 1.5  # Softer targets to pass ranking signal beyond the argmax token
...
class LatentPredictor(nn.Module):
    """Small MLP to predict next token's h_final from current h_source."""
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # EXP-029.2: Latent predictor for next-token prediction
    ...
    def init_weights(self):
        ...
        # EXP-029.2: Init latent predictor
    ...
    def forward_head(self, x, targets=None, reduction='mean', intermediate=None, return_components=False):
        ...
        if targets is not None:
            ...
            # EXP-029.2.5: distill next-token logits rather than the full hidden geometry
            if intermediate is not None and intermediate.size(1) > 1:
                predicted_next = self.latent_predictor(intermediate[:, :-1].float())
                predicted_logits = self.lm_head(predicted_next).float()
                predicted_logits = softcap * torch.tanh(predicted_logits / softcap)
                target_logits = logits[:, 1:].detach()
                target_probs = F.softmax(target_logits / DISTILL_TEMPERATURE, dim=-1)
                log_probs = F.log_softmax(predicted_logits / DISTILL_TEMPERATURE, dim=-1)
