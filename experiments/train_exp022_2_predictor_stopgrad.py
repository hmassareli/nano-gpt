...
"""
EXP_TITLE = "EXP-022.2: Predictor + Stop-Grad Embedding Bypass"
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.embedding_predictor = EmbeddingPredictor(config.n_embd, PREDICTOR_HIDDEN_DIM)
        # Value embeddings
    ...
    def init_weights(self):
        ...
        torch.nn.init.uniform_(self.embedding_predictor.fc1.weight, -s, s)
        torch.nn.init.zeros_(self.embedding_predictor.fc2.weight)
        # Value embeddings
    ...
    def num_scaling_params(self):
        ...
        predictor = sum(p.numel() for p in self.embedding_predictor.parameters())
        scalars = self.resid_lambdas.numel() + self.x0_lambdas.numel()
        total = wte + value_embeds + lm_head + transformer_matrices + predictor + scalars
        return {
            'wte': wte, 'value_embeds': value_embeds, 'lm_head': lm_head,
            'transformer_matrices': transformer_matrices, 'predictor': predictor,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        predictor_params = list(self.embedding_predictor.parameters())
    ...
    def forward_head(self, x, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            # EXP-022.2: predictor + stop-grad target embedding bypass.
            ...
            if valid_mask.any():
                ...
                predicted_embs = self.embedding_predictor(flat_hidden[valid_mask])
                emb_sq_error = (predicted_embs - target_embs[valid_mask]).pow(2).mean(dim=-1)
                if reduction == 'sum':
...
# EXP-022 specific
EMB_LOSS_LAMBDA = 0.1    # weight for predictor-based embedding bypass
